jQuery( document ).ready( function() {
	var active = 0;
	if( jQuery.cookie( 'ecvetstep_ad_tabs' ) ) {
		active = jQuery.cookie( 'ecvetstep_ad_tabs' );
		jQuery.cookie( 'ecvetstep_ad_tabs', null );
	}
	
	var tabs = jQuery( '#ecvetstep_ad_tabs' ).tabs( { active: active } );
	
	jQuery( '#wpbody-content form' ).submit( function() {
		var active = tabs.tabs( 'option', 'active' );
		jQuery.cookie( 'ecvetstep_ad_tabs', active );
	} );
